...
# Extending periods as Poloniex seems to lagging by a day
if tickPeriod == fiveMinsInSeconds:
  secondsToSubtract = oneDayInSeconds / 2
elif tickPeriod == fifteenMinsInSeconds:
  secondsToSubtract = oneDayInSeconds / 1
elif tickPeriod == fourHoursInSeconds:
  secondsToSubstract = oneDayInSeconds * 6
...
# Offset 25200 is 7 hours as now running on SGT time from laptop
#startTimeEpoch = int(endTimeEpoch) - secondsToSubtract - 25200
startTimeEpoch = int(endTimeEpoch) - secondsToSubtract

print "StartTimeEpoch: " + str(startTimeEpoch)
...
def downloadPrices():
    ...
    print
    print "From Date: " + time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(startTimeEpoch))
    print "To Date: " + time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(endTimeEpoch))
    print
...
def calcADX():
    ...
    for keyPair in btcTickPairs:
        ...
        if myTuple is not None:
            ...
            alreadyPotential = [x[0] for x in existingPotOrAct]

            candleColours = dbMod.getGreen(keyPair)
            for tuples in candleColours:
                openP = tuples[0]
                closeP = tuples[1]
                if (closeP > openP):
                    noOfGreen +=1
            ...
            if (noOfGreen > 1 and direction == 'BUY'):
                green = True
            if (noOfGreen < -1 and direction == 'SELL'):
                green = True

            #pairVolTuple = dbMod.getVolume(keyPair)
            #pairVol = pairVolTuple[0][0]
            #print pairVol
            #if pairVol > 2:
            #    enoughVolume = True
            enoughVolume = True

            if keyPair not in alreadyPotential and green and enoughVolume:
                #print "Inserting a new Potential " + keyPair + ", noOfGreen = " + str(noOfGreen) + ", volume = " + str(pairVol) + " at time " + str(lastTime)
                print "Inserting a new Potential " + keyPair + ", noOfGreen = " + str(noOfGreen) + " at time " + str(lastTime)
                dbMod.insertPotential(myTuple)
            else:
                #print "NOT Inserting a new " + direction + " Potential " + keyPair + ", noOfGreen = " + str(noOfGreen) + ", volume = " + str(pairVol)
                print "NOT Inserting a new " + direction + " Potential " + keyPair + ", noOfGreen = " + str(noOfGreen)
...
def checkStopLimit():
    ...
    for pairActive, activeType, utBought, direction, startPrice, amount, stopPrice, limitPrice in actives:
		...
		if (direction == 'BUY'):
			...
			print pairActive + "...We've gone past our BUY LIMIT, current price is  " + str(lastPriceFloat) + " and it's above our limit of " + str(priceLimitFloat) + " - profit so far = " + str(profit) + ", startPrice= " + str(startPrice)
			#sendSMS.sendMessage(pairActive + "...We've hit our BUY LIMIT of " + str(lastPriceFloat) + " as it's above our limit of " + str(priceLimitFloat) + " - cashing out, profit = " + str(profit) + ", startPrice= " + str(startPrice))
		        ...
		        #dbMod.insertHistory(pairActive, activeType, utBought, nowTimeEpoch, direction, startPrice, lastPrice, amount, profit)
                        ...
                        distanceAboveLimit = lastPriceFloat - priceLimitFloat
                        interimStop = priceStopFloat + distanceAboveLimit
                        newLimit = lastPriceFloat + (lastPriceFloat * LIMIT_WEIGHT)
                        newStop = interimStop + (interimStop * STOP_WEIGHT)
...
def checkBuyTriggers():
	...
	for key, value in polTicker.iteritems():
		...
		#if lastPriceFloat >= priceTriggerFloat and lastPriceFloat < triggerCeiling:
		#if lastPriceFloat >= priceTriggerFloat and lastPriceFloat < limit:
		if lastPriceFloat >= priceTriggerFloat and lastPriceFloat < highPriceFloat:

    		    #if ( noOfCoinsToBuy * lastPriceFloat <= totalCoin ):
    		    if ( True ):
                        #print "Buy %d of %s at %s" % (noOfCoinsToBuy, pairLatest, priceFloat)
                        print "Buy %d of %s at %s, above trigger price of %s, and below highPriceFloat" % (noOfCoinsToBuy, pairLatest, lastPriceFloat, priceTriggerFloat, highPriceFloat)
