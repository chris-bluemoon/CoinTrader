...
def resetPotential(pair, newPrice, newTime):

    conn = psycopg2.connect(**params)
    cur = conn.cursor()

    try:
        command= "UPDATE POTENTIALS SET adx_start_ut = " + str(newTime) + ", start_price = " + str(newPrice) + ", low_price = 0, high_price = 0 WHERE pair = '" + pair + "'"
        #command= "INSERT INTO balances (exchange, coin, amount) VALUES ('" + exchange + "', '" + coin + "', " + str(amount) + ")" + " ON CONFLICT ON CONSTRAINT balances_pkey DO UPDATE SET amount  = " + str(amount)
        cur.execute(command)
	conn.commit()
    except (Exception, psycopg2.DatabaseError) as error:
        print(error)
        print "End of error detection in resetPotential"
    finally:
        if conn is not None:
            conn.close()
...
def getGreen(keyPair):

    conn = psycopg2.connect(**params)
    cur = conn.cursor()

    query = "SELECT open, close from ohlcv where pair = '" + keyPair + "'"
    cur.execute(query,)
    rows = cur.fetchall()
...
def getVolume(keyPair):

    conn = psycopg2.connect(**params)
    cur = conn.cursor()

    query = "SELECT sum(volume) from ohlcv where pair = '" + keyPair + "'"
    cur.execute(query,)
    rows = cur.fetchall()
